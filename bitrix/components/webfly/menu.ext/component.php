        ...
        if ($arParams["DEPTH_LEVEL_FINISH"] >= 2) {
          ...
          while ($arRes = $res->GetNext()) {
            ...
            if ($arParams["DEPTH_LEVEL_FINISH"] >= 3) {
              ...
              $resSect = CIBlockSection::GetList(Array("left_margin" => "asc"), $arFilter, $bIncCnt, array("UF_*"));
              while ($arResSect = $resSect->GetNext()) {
                ...
                if ($arParams["DEPTH_LEVEL_START"] <= 3) {
                  ...
                  $arResult[$i][3] = array("FROM_IBLOCK" => "1", "IS_PARENT" => "", "DEPTH_LEVEL" => $depthLevel, "PICTURE" => CFile::GetPath($arResSect["PICTURE"]), "ELEMENT_CNT" => $cnt, "CHILD_SECTION_ID" => $child, "PIC_LINK" => $arResSect["UF_PIC_LINK"]);
                  if ($level == 1) {
