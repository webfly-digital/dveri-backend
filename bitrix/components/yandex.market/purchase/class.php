...
class Purchase extends \CBitrixComponent
{
	...
	public function onPrepareComponentParams($arParams)
	{
		...
		$arParams['SERVICE_CODE'] = trim($arParams['SERVICE_CODE']);
		$arParams['SITE_ID'] = trim($arParams['SITE_ID']);
		$arParams['URL_ID'] = trim($arParams['URL_ID']);
	...
	public function executeComponent()
	{
		...
		{
			list($status, $response) = $this->processException($exception, $logger);
		}
	...
	{
		return Main\Application::getInstance()->getContext()->getServer();
	}

	protected function processException($exception, Market\Psr\Log\LoggerInterface $logger = null)
	{
	 ...
	 */
	protected function logException($exception, Market\Psr\Log\LoggerInterface $logger = null)
	{
