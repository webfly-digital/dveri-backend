				...
				switch ($filter['type'])
				{
					...
					case 'list':
						...
						<select name="<?= $filter['id'] ?>" id="<?= $filter['id'] ?>">
							...
							{
								if (isset($optionValue['LEGEND']))
								{
									if ($optgroupOpen) { echo '</optgroup>'; }

									$optgroupOpen = true;
									?>
									<optgroup label="<?= Market\Utils::htmlEscape($optionValue['NAME']) ?>">
									<?php
									continue;
								}

								?>
								<option value="<?= $optionId ?>" <?= (string)$optionId === (string)$filter['value'] ? 'selected' : '' ?>><?= Market\Utils::htmlEscape($optionValue['NAME']) ?></option>
								<?php
							}
