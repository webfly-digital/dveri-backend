...
if ($arResult["PROPERTIES"]["DOP_COMPLECTATION"]["VALUE"]) {
    ...
    $cmplct = CIBlockElement::GetList([], ["IBLOCK_ID" => WF_ADDITIONAL_COMPLECT, "ID" => $arResult["PROPERTIES"]["DOP_COMPLECTATION"]["VALUE"], "ACTIVE" => "Y"], false, false, ["ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "PROPERTY_PRICE"]);
    while ($ob_cmplct = $cmplct->Fetch()) {
        if ($ob_cmplct["PREVIEW_PICTURE"]) {
            $ob_cmplct["PHOTO"] = CFile::ResizeImageGet($ob_cmplct["PREVIEW_PICTURE"], ["width" => 65, "height" => 65], BX_RESIZE_IMAGE_EXACT, true);
        }
...
//УТП START
$utp = CIBlockElement::GetList(['sort' => 'asc'], ["IBLOCK_ID" => WF_DETAIL_UTP, "ACTIVE" => "Y"], false, false, ["ID", "NAME", "PREVIEW_TEXT", "PROPERTY_LINK", "PROPERTY_MEDIALIBRARY"]);
...
if (count($arResult["MERGE_PHOTOS"]) > 0) {
    foreach ($arResult["MERGE_PHOTOS"] as $pkey => $photo) {
        $curPhotoBig = CFIle::ResizeImageGet($photo["ID"], ["width" => 900, "height" => 900], BX_RESIZE_IMAGE_PROPORTIONAL, true);
        if (!empty($photo["ALT"]))
        ...
        $arResult["PHOTOS"]["SMALL"][] = CFIle::ResizeImageGet($photo["ID"], ["width" => 500, "height" => 500], BX_RESIZE_IMAGE_PROPORTIONAL, true);
    }
}
...
if ($arResult["IBLOCK_SECTION_ID"]) {
    $colors = CIBlockSection::GetList([], ["IBLOCK_ID" => $arResult["IBLOCK_ID"], "ID" => $arResult["IBLOCK_SECTION_ID"], "ACTIVE" => "Y"], false, ["IBLOCK_ID", "ID", "UF_COLORS"], false)->Fetch();

    $clrs_res = CIBlockElement::GetList([], ["IBLOCK_ID" => WF_COLORS, "ACTIVE" => "Y", "ID" => $colors["UF_COLORS"]], false, false, ["ID", "NAME", "PREVIEW_PICTURE", "PROPERTY_DOP_NAME"]);
    while ($ob_clrs_res = $clrs_res->Fetch()) {
        if ($ob_clrs_res["PREVIEW_PICTURE"]) {
            $ob_clrs_res["PHOTO"] = CFile::ResizeImageGet($ob_clrs_res["PREVIEW_PICTURE"], ["width" => 60, "height" => 60], BX_RESIZE_IMAGE_EXACT, true);
        }
