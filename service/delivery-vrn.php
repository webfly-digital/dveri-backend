<?php require($_SERVER["DOCUMENT_ROOT"] . "/bitrix/header.php");

/** @global CMain $APPLICATION */

$APPLICATION->SetPageProperty("DESCRIPTION", "Установка и монтаж противопожарных дверей в #WF_CITY_PRED#");
$APPLICATION->SetPageProperty("title", "Доставка и оплата | Завод «Двери металл-М» в #WF_CITY_PRED#");
$APPLICATION->SetTitle("Доставка и оплата (Воронеж)");
?>

    <div class="text-content">
        <p class="subtitle">Компания Двери Металл-М организует доставку заказа на указанный адрес по Москве и за пределы МКАД. Оформление доставки происходит после подтверждения заказа менеджером и
            согласования стоимости и сроков. Совместно с клиентом подбирается удобная дата и время привоза. Доставляются заказы любых объемов. Стоимость составляет от 4500 рублей в зависимости от
            объема заказа и расстояния для транспортировки.</p>
    </div>

    <!--Оплата и предоплата-->
    <section class="page-section">
        <p class="h2 page-section__title">Оплата и предоплата</p>
        <p class="subtitle">Счет на оплату выставляется после заключения договора о поставке изделий. Оплату возможно произвести любым удобным для клиента способом:</p>
        <div class="text-content">
            <!--Варианты оплаты-->
            <div class="row other-content">
                <div class="col-xs-12 col-sm-6">
                    <div class="ico-block theme--light-gray">
                        <div class="ico-block__inner">
                            <div class="ico-block__icon">
                                <svg class="svg-icon svg-icon--xxl">
                                    <use xlink:href="/bitrix/templates/1dvm/img/svg-symbols.svg#cash"></use>
                                </svg>
                            </div>
                            <div class="ico-block__content">
                                <p class="ico-block__title">Наличные</p>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="col-xs-12 col-sm-6">
                    <div class="ico-block theme--light-gray">
                        <div class="ico-block__inner">
                            <div class="ico-block__icon">
                                <svg class="svg-icon svg-icon--xxl">
                                    <use xlink:href="/bitrix/templates/1dvm/img/svg-symbols.svg#cashless"></use>
                                </svg>
                            </div>
                            <div class="ico-block__content">
                                <p class="ico-block__title">Безналичный расчет</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <!--/варианты оплаты-->

            <p>При оплате по безналичному расчету предоплата составляет 100%. При крупных объемах заказа возможны индивидуальные условия. Чтобы рассчитать стоимость по индивидуальному предложению,
                свяжитесь с менеджером компании.</p>

            <h3>Порядок оплаты при наличном расчете</h3>
            <p>После заключения договора на производство и поставку продукции оплата должна быть внесена наличными менеджеру компании или мастеру-замерщику. На руки клиенту выдается квитанция о
                произведенной оплате.</p>

            <h3>Порядок оплаты при безналичном расчете</h3>
            <p>После заключения договора на производство и поставку клиенту выставляется счет на оплату. Счет отправляется любым удобным для покупателя способом — факс, электронная почта, заказное
                письмо и прочее. Оплату указанной суммы необходимо произвести в полном объеме.</p>

            <div class="gal gal-v4">
                <div class="gal-item">
                <a href="/bitrix/templates/1dvm/img/doc-examples/doc-1.jpg" class="gal-item__preview">
                    <img alt="" src="<?= ImageCompressor::getCompressedSrc('/bitrix/templates/1dvm/img/doc-examples/preview/doc-1.jpg') ?>">
                </a>
                </div>
                <div class="gal-item">
                <a href="/bitrix/templates/1dvm/img/doc-examples/doc-2.jpg" class="gal-item__preview">
                    <img alt="" src="<?= ImageCompressor::getCompressedSrc('/bitrix/templates/1dvm/img/doc-examples/preview/doc-2.jpg') ?>">
                </a>
                </div>
                <div class="gal-item">
                <a href="/bitrix/templates/1dvm/img/doc-examples/doc-3.jpg" class="gal-item__preview">
                    <img alt="" src="<?= ImageCompressor::getCompressedSrc('/bitrix/templates/1dvm/img/doc-examples/preview/doc-3.jpg') ?>">
                </a>
                </div>
            </div>
            <p class="text-caption">Пример документов, которыми сопровождается каждый заказ</p>
        </div>
    </section>
    <!--/оплата и предоплата-->

    <!--Варианты доставки-->
    <section class="page-section">
        <h2 class="page-section__title">Варианты доставки</h2>
        <div class="text-content">
            <h3>Собственная доставка</h3>
            <p>Доставка по Воронежу осуществляется при помощи собственного транспорта компании, что позволяет организовать быструю перевозку любых объемов заказа на указанный адрес.</p>

            <h3>Самовывоз</h3>
            <p>Также возможно оформление самовывоза с территории завода по адресу ул. Богдана Хмельницкого, д. 77а. Для оформления самовывоза сообщите о желаемом способе получения товара менеджеру при
                согласовании заказа.</p>
            <p>
                <a href="#modalUniversal" class="btn btn--gray modal-universal-button" data-options="Вызвать замерщика|.input-comment"><span class="flaticon-user-1"></span> Вызвать замерщика</a>
            </p>

        </div>
    </section>
    <!--/варианты доставки-->

    <!--Условия доставки-->
    <section class="page-section">
        <h2 class="page-section__title">Условия доставки</h2>
        <div class="text-content">
            <p class="subtitle">Организация доставки начинается после поступления полной оплаты заказа. Стоимость доставки зависит от объема заказанной партии и от удаленности объекта. Средняя
                стоимость доставки по Воронежу:</p>

            <ul class="">
                <li>от 1500 рублей при заказе 1 и более позиций,</li>
                <li>от 2500 рублей при заказе 10 и более позиций.</li>
            </ul>
            <p>Доставка производится в указанную дату и время, предварительно согласованные с клиентом. Согласование производится при оформлении заказа или по факту готовности продукции.</p>
        </div>
    </section>
    <!--/условия доставки-->

    <!--Фото/видео-->
    <section class="page-section">
        <h2 class="page-section__title">Фото и видео с производства</h2>
        <div class="text-content">
            <p class="subtitle">Приглашаем вас детальнее ознакомиться с процессом производства нашей продукции</p>
            <div class="embed-responsive embed-responsive-16by9">
                <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/SXc0VjE_qFY"></iframe>
            </div>

            <!--Фотки с производства - ограничиться тремя-->
            <!--Gallery-->
            <div class="gal gal-v2">
                <div class="gal-item">
                <a href="/bitrix/templates/1dvm/img/gal/04.jpg" class="gal-item__preview"
                   style="background-image: url('<?= ImageCompressor::getCompressedSrc('/bitrix/templates/1dvm/img/gal/04.jpg') ?>');">
                </a>
                </div>
                <div class="gal-item">
                <a href="/bitrix/templates/1dvm/img/gal/07.jpg" class="gal-item__preview"
                   style="background-image: url('<?= ImageCompressor::getCompressedSrc('/bitrix/templates/1dvm/img/gal/07.jpg') ?>');">
                </a>
                </div>
                <div class="gal-item">
                <a href="/bitrix/templates/1dvm/img/gal/08.jpg" class="gal-item__preview"
                   style="background-image: url('<?= ImageCompressor::getCompressedSrc('/bitrix/templates/1dvm/img/gal/08.jpg') ?>');">
                </a>
                </div>
            </div>
            <!--/gallery-->
            <!--/фотки с производства-->
        </div>
    </section>
    <!--/фото/видео-->


<?php require($_SERVER["DOCUMENT_ROOT"] . "/bitrix/footer.php"); ?>